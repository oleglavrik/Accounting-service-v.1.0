<?php 
	/*
		[НЕ АРХІВ]
		* Скрипт редагування інформації про товар на складі,
		* зміненої вже користуваче, це запис в БД вже редагованої інформації про продукт
	*/
	# скрипт security
	...
	$editInfoQuery = "UPDATE store SET 
										...
										weight = '".mysql_real_escape_string($productSimple)."',
	...
	$updatingInfo = "SELECT typesofproducts.idProductType, store.idShop, store.productName, store.dateOfPurchase, store.inputPrice, store.startingPrice, store.itemType, store.id, store.barCode, store.productSimple, store.weight, store.size, typesofproducts.productTypesDescription FROM store LEFT JOIN typesofproducts ON store.itemType = typesofproducts.idProductType WHERE store.dateOfSale = '0000-00-00' and store.archiveStatus = '0' and store.idShop = '".mysql_real_escape_string($idShop)."' ORDER BY store.productName";
